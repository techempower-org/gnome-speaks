...
| `GET /status` | Service state, pause flag, `quiet` (quiet hours: `active`, `enabled`, `scheduled`, `window`, `override`, `until` — while active, `POST /speak` answers 503; your own speech is never muted; override with "cast quiet hours" or the panel switch), `extension` (`false` while the GNOME Shell extension is disabled — the service then refuses to listen, speak or type: `POST /speak` answers 503, queued items end `suppressed`), `queue_depth`, playback progress, and `speech` — the live provider route (`backend`, `offline_reason`, `local_down`, `forced_offline`, `wyoming_configured`; see [Speech backend](#speech-backend)). |
